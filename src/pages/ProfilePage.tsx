...
import {
  ...
  ShoppingBag, Camera, Moon, Sun, Wifi, Mail, Save, CheckCircle,
  AlertCircle, ChevronDown, MapPin, Phone, Trash2, Palette,
} from 'lucide-react';
import { ListingCard } from '../components/ListingCard';
import type { Listing, Profile } from '../types';
import { getMyListings, getFavorites, getCurrentUser, getProfile, signOut, updateProfile, deleteAccount } from '../lib/api';
...
export function ProfilePage() {
  ...
  const [user, setUser] = useState<ReturnType<typeof getCurrentUser> extends Promise<infer T> ? T : never | null>(null);
  ...
  useEffect(() => {
    getCurrentUser().then((u) => {
      ...
      if (u) {
        ...
        setLoading(true);
        Promise.all([getMyListings(), getFavorites()])
          .then(([listings, favs]) => {
  ...
  const themes: { id: ThemeMode; label: string; desc: string; icon: typeof Sun }[] = [
    ...
    { id: 'kamba',  label: 'Modo Kamba', desc: 'Segue o tema do teu dispositivo', icon: Palette },
  ];
        ...
        <div className="max-w-2xl mx-auto px-4 py-8">
          ...
          {settingsSection === 'profile' && (
            <div className="p-6 rounded-2xl bg-white/5 border border-white/10 space-y-5">
              <h2 className="text-lg font-semibold text-white flex items-center gap-2">
                <User className="w-5 h-5" /> Gestão de Perfil
              </h2>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    >
                      <option value="" className="bg-background-card">Selecionar</option>
                      {(MUNICIPIOS[editProvincia] || []).map((m) => (
              ...
              >
                {saving ? (
                  <><div className="w-5 h-5 rounded-full border-2 border-white/30 border-t-white animate-spin" />A guardar...</>
                ) : (
              ...
              {saveSuccess && (
                <div className="flex items-center gap-2 text-mercado text-sm">
                  <CheckCircle className="w-4 h-4" /> Perfil actualizado com sucesso!
                </div>
              )}
            </div>
          )}
          ...
          {settingsSection === 'tema' && (
            <div className="p-6 rounded-2xl bg-white/5 border border-white/10 space-y-5">
              ...
              <p className="text-white/60 text-sm">
                Escolhe como o KazaKamba aparece no teu dispositivo.
              </p>
              ...
              <div className="flex items-center gap-3 p-3 rounded-xl bg-white/5 border border-white/10">
                <div className={`w-3 h-3 rounded-full flex-shrink-0 ${isDark ? 'bg-neutral-700' : 'bg-yellow-300'}`} />
                <p className="text-white/60 text-sm">
                  Modo actual: <span className="text-white font-medium">{isDark ? 'Escuro' : 'Claro'}</span>
                </p>
              </div>
            </div>
          )}
          ...
          {settingsSection === 'data' && (
            <div className="p-6 rounded-2xl bg-white/5 border border-white/10 space-y-6">
              <h2 className="text-lg font-semibold text-white flex items-center gap-2">
                <Wifi className="w-5 h-5" /> Modo Poupança de Dados
              </h2>
              <p className="text-white/60 text-sm">
                Quando activado, reduz o consumo de dados móveis bloqueando o autoplay de vídeos nos Stories e reduzindo a qualidade das imagens.
              </p>

              <div className="flex items-center justify-between p-4 rounded-xl bg-white/5 border border-white/10">
                <div className="flex items-center gap-3">
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${dataSaver ? 'bg-mercado/20' : 'bg-white/10'}`}>
                    <Moon className={`w-6 h-6 ${dataSaver ? 'text-mercado' : 'text-white/40'}`} />
                  </div>
                  <div>
                ...
                <button
                  ...
                  className={`w-14 h-8 rounded-full relative transition-all ${dataSaver ? 'bg-mercado' : 'bg-white/20'}`}
              ...
              <div className="space-y-3">
                {[
                  { label: 'Autoplay de vídeos bloqueado', desc: 'Os vídeos nos Stories não serão reproduzidos automaticamente' },
                  { label: 'Imagens optimizadas', desc: 'Resolução das imagens reduzida para poupar dados' },
                ].map(({ label, desc }) => (
                  <div key={label} className="flex items-start gap-3 p-3 rounded-xl bg-white/5">
                    {dataSaver ? <CheckCircle className="w-5 h-5 text-mercado mt-0.5" /> : <AlertCircle className="w-5 h-5 text-white/40 mt-0.5" />}
                    <div>
          ...
          {settingsSection === 'suggestions' && (
            <div className="p-6 rounded-2xl bg-white/5 border border-white/10 space-y-5">
              <h2 className="text-lg font-semibold text-white flex items-center gap-2">
                <Mail className="w-5 h-5" /> Caixa de Sugestões
              </h2>
              <p className="text-white/60 text-sm">
                Tem alguma ideia para melhorar o KazaKamba? Envie a sua sugestão e ajude-nos a construir a melhor plataforma para Angola!
              </p>

              {suggestionSent ? (
                <div className="text-center py-8">
                  <div className="w-16 h-16 rounded-full bg-mercado/20 flex items-center justify-center mx-auto mb-4">
                    <CheckCircle className="w-8 h-8 text-mercado" />
                  </div>
                  <p className="text-white font-medium mb-2">Obrigado pela sua sugestão!</p>
                  <p className="text-white/60 text-sm">A sua ideia foi enviada para a nossa equipa.</p>
                </div>
              ) : (
                <div className="space-y-4">
                  <div>
                    <label className="block text-white/70 text-sm mb-2">A sua sugestão</label>
                    <textarea
                  ...
                  <div className="flex items-center gap-2 text-white/50 text-xs">
                    ...
                    <span>Sugestões enviadas para: Edivaldotc16@gmail.com</span>
                  </div>
                  <button
                  ...
                  >
                    {sendingSuggestion ? (
                      <><div className="w-5 h-5 rounded-full border-2 border-white/30 border-t-white animate-spin" />A enviar...</>
                    ) : (
                      <><Mail className="w-5 h-5" />Enviar Sugestão</>
                    )}
                  </button>
                </div>
              )}
            ...
            <div className="space-y-4">
              ...
              <div className="p-6 rounded-2xl bg-white/5 border border-white/10">
                ...
                <p className="text-white/60 text-sm mb-4">
                  Sai da tua conta neste dispositivo. Os teus dados ficam guardados.
                </p>
                <button
              ...
              <div className="p-6 rounded-2xl bg-red-500/5 border border-red-500/20">
                ...
                <p className="text-white/60 text-sm mb-4">
                  Esta acção é <strong className="text-red-400">permanente e irreversível</strong>. Todos os teus anúncios, mensagens,
                  stories e dados de perfil serão eliminados. Não é possível recuperar a conta depois.
                </p>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-start gap-3 p-4 rounded-xl bg-red-500/10 border border-red-500/30">
                      ...
                      <p className="text-red-300 text-sm font-medium">
                        Tens a certeza absoluta? Esta acção não pode ser desfeita.
                      </p>
                    </div>
                    ...
                    <div className="flex gap-3">
                      ...
                      >
                        {deletingAccount ? (
                          <><div className="w-5 h-5 rounded-full border-2 border-white/30 border-t-white animate-spin" />A eliminar...</>
                        ) : (
                          <><Trash2 className="w-5 h-5" />Sim, eliminar conta</>
                        )}
                      </button>
                    </div>
                  </div>
  ...
  return (
    <div className="min-h-screen bg-background py-8">
      <div className="max-w-4xl mx-auto px-4">
        <div className="p-6 rounded-2xl bg-gradient-to-b from-background-card to-background border border-white/10 mb-8">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-white mb-1">{profile?.nome || 'Utilizador'}</h1>
                <div className="flex items-center gap-2 text-white/60 text-sm">
                  <MapPin className="w-4 h-4" />
                  <span>{profile?.bairro}, {profile?.municipio}</span>
                </div>
                {profile?.verificado && (
                  <span className="inline-flex mt-2 px-2 py-0.5 rounded bg-mercado/20 text-mercado text-xs items-center gap-1">
          ...
          <div className="grid grid-cols-3 gap-4 mt-6">
            ...
            <div className="text-center p-4 rounded-xl bg-white/5">
              <p className="text-2xl font-bold text-mercado">{favorites.length}</p>
        ...
        {loading ? (
          ...
          favorites.length > 0 ? (
            ...
            <div className="text-center py-16 bg-white/5 rounded-2xl border border-white/10">
              ...
              <p className="text-white/60 mb-6">Explore anúncios e guarde os seus favoritos</p>
              <div className="flex flex-col sm:flex-row gap-3 justify-center">
                ...
                <Link to="/mercado" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-mercado text-white font-medium hover:shadow-lg hover:shadow-mercado/30 transition-all">
